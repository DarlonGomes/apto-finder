...
| 6 | SPA v1 | DONE (list, cost bar + legend, filter sheet/modal, swipe triage, PWA). Status v2: append-only `status_events` (liked / visit_booked / proposal_made / dismissed) with actor from the `Cf-Access-Authenticated-User-Email` header; current status = latest event, undo deletes it. visit_booked carries `visit_at`, proposal_made carries `amount_cents` + optional `note` (inline forms on the card). Old `unit_status` table: migration `0004_drop_unit_status.sql` is written but NOT applied (permission-gated in the session that wrote it); run `pnpm --filter collector migrate`. |
...
| 8 | Daily digest | BLOCKED on PRD open question 4: which channel (email needs a provider account, Telegram needs a bot token, WhatsApp likewise; ntfy.sh would need the app installed). Decide the channel, then it's a small collector step after the sweep. |
